...
const student = useCookie("kollel_sys_student");
const hasAccess = useCookie("kollel_sys_has_access");

const schema = object({
phone: string() .transform(v => v.replace(/\D/g, "")) // clean: remove (), -, spaces 
.matches(/^\d{10}$/, "Invalid Phone Number") 
.required("Phone is required"),  

password: string()
    .min(1, "Must be at least 4 characters")
...
const onSubmit = async (event) => {
  try {
    ...
    const response = await api("/api/student-portal/login", {
      method: "POST",
      body: event.data,
    });
    //getitem, json.parse
